...
- [ ] 영화 목록의 1페이지를 불러오며 페이지끝에 도달하면 그 다음의 영화 목록을 불러 올 수 있다.
...
- [ ] 영화 포스터나 제목 또는 메인 화면의 자세히 보기를 누른경우 영화 상세 정보를 모달로 출력한다.
  - [ ] 닫기 버튼 또는 ESC를 누르면 모달이 닫힌다.
  - [ ] 모달창을 반응형 레이아웃으로 구현한다.
- [ ] 사용자는 영화에 대해 별점을 남길 수 있다
  - [ ] 별점은 5개로 구성되어있으며 한개장 2점이다, 1점 단위는 고려하지 않는다.
  - [ ] 새로고침하더라도 별점은 유지되어야 한다
  - [ ] localStorag로 구현하돼 서버 API로 쉽고 안전하게 갈아끼울 수 있는 구조로 개발하여야한다.
