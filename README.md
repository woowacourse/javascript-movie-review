# javascript-movie-review

FE 레벨1 영화 리뷰 미션

## 요구사항 명세서

### 영화

- [x] 영화 리스트를 불러온다.
- [x] 영화 섹션 하나에 포스터, 평점, 영화 제목을 띄운다.
- [x] 영화 목록을 띄울 때 20개씩 띄운다.
- [x] 검색어에 따른 필러링된 영화 리스트를 불러온다.

- [x] 영화를 불러오는 도중에는 스켈레톤 UI를 보여준다.
- [x] 포스터 클릭 시 백그라운드 영역에 해당 영화 포스터를 보여준다.

### 더 보기 기능

- [x] '더보기' 버튼을 누르면 영화 20개를 추가로 띄운다. [요구사항 변경]
- [ ] 더 보기 기능을 페이징하는 방식에서 무한 스크롤 방식으로 변경한다.

### 검색 기능

- [x] 검색란에 영화 제목을 입력해서 검색 버튼 또는 Enter키를 눌러 필터링된 영화 20개를 띄운다.
- [x] 검색 결과가 존재하지 않으면 "검색 결과가 없습니다" 텍스트를 띄운다.
- [x] 검색 결과 화면에서 검색란에 검색어가 존재하지 않은 상태에서 검색 버튼 또는 Enter키를 눌러 인기순 영화 페이지로 돌아간다.

### 모달

- [ ] 영화 포스터나 제목을 클릭하면 영화 세부정보를 모달로 띄운다.
- [ ] 모달에는 포스터, 장르, 평점, 내 별점, 줄거리가 포함된다.
- [ ] 닫기 버튼 또는 ESC키를 눌러 모달을 닫는다.

### 별점 기능

- [ ] 세부정보 모달에서 사용자가 별점을 매길 수 있고,
- [ ] 별점은 5개로 구성되어 있으며 한 개당 2점이며 1점 단위는 고려하지 않는다.
  - 2점: 최악이예요
  - 4점: 별로예요
  - 6점: 보통이에요
  - 8점: 재미있어요
  - 10점: 명작이에요
- [ ] 사용가자 페이지를 새로고침하더라도 매긴 별점은 유지한다.

### 에러 처리

- [x] 초기 영화 목록 로딩에 실패하면 에러 메세지를 보여준다.
- [x] 검색 요청이 실패하면 검색 에러 메세지를 보여준다.
- [x] 더 보기 요청이 실패하면 추가 로딩 에러 메세지를 보여준다.

### 반응형 디자인

- [ ] 데스크톱 화면의 경우 페이지 가로 사이즈에 따라 한 줄에 표시되는 영화의 수가 최소 4개, 최대 5개이다.
- [ ] 데스크톱 화면의 경우 모달이 화면 정 가운데 표시된다.
- [ ] 태블릿 화면의 경우 페이지 한 줄에 표시되는 영화의 수가 3개이다.
- [ ] 태블릿 화면의 경우 모달이 화면 하단에 표시된다.
- [ ] 모바일 화면의 경우 페이지 한 줄에 표시되는 영화의 수가 1개이다.
- [ ] 모바일 화면의 경우 포스터를 제외한 모달이 화면 하단에 표시된다.

## 테스트 시나리오

### 인기순 영화 페이지 흐름

- [x] 페이지 진입 시 인기순 영화 목록이 표시되고, 첫 번째 영화가 배경에 표시된다.
- [x] 다른 포스터를 클릭하면 해당 영화로 배경이 갱신된다.
- [x] 더 보기 버튼을 누르면 다음 페이지 영화가 기존 목록 뒤에 추가된다.

### 영화 검색 결과 페이지 흐름

- [x] 검색어를 입력하고 검색을 실행하면 필터링된 영화 목록을 표시한다.
- [x] 검색 결과가 없으면 "검색 결과가 없습니다" 메시지를 표시한다.
- [x] 검색 결과 화면에서 더 보기 버튼을 누르면 다음 페이지 영화가 기존 목록 뒤에 추가된다.

### 세부정보 모달 & 별점 기능 흐름

- [ ] 영화 포스터 또는 제목을 클릭하면 세부 정보 모달을 띄울 수 있고, 별점을 매길 수 있다.
- [ ] 닫기 버튼 또는 ESC키를 눌러 모달을 닫을 수 있고, 새로고침해도 사용자가 매긴 별점이 유지된다.

### 에러 메시지 흐름

- [x] 초기 영화 목록 로딩에 실패하면 에러 메시지를 보여준다.
- [x] 검색 요청이 실패하면 검색 에러 메시지를 보여준다.
- [x] 더 보기 요청이 실패하면 추가 로딩 에러 메시지를 보여준다.
