...
describe("인기순 영화 페이지에서 영화 포스터를 클릭하면 해당 영화 정보를 백그라운드에 보여준다.", () => {
  ...
  it("인기순 영화 페이지에서 두 번째 포스터를 클릭하면 백그라운드에 해당 영화가 배경에 표시된다.", () => {
    cy.get(".thumbnail-list li").eq(1).click();

    cy.get(".top-rated-movie .title").should("have.text", "인기 영화 2");
  });
  it("인기순 영화 페이지에서 다섯 번째 포스터를 클릭하면 백그라운드에 해당 영화가 배경에 표시된다.", () => {
    cy.get(".thumbnail-list li").eq(4).click();

    cy.get(".top-rated-movie .title").should("have.text", "인기 영화 5");
  });
  it("인기순 영화 페이지에서 열두 번째 포스터를 클릭하면 백그라운드에 해당 영화가 배경에 표시된다.", () => {
    cy.get(".thumbnail-list li").eq(11).click();

    cy.get(".top-rated-movie .title").should("have.text", "인기 영화 12");
  });
