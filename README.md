...
## [시나리오 10] 무한 스크롤 중 API 실패 시 스크롤 트리거 유지 - 인기영화

User -> Main Page -> 스크롤하여 페이지 끝에 도달 -> 다음 페이지 API 실패 -> 토스트 에러 표시 -> 스크롤 트리거가 사라지지 않고 유지된다

## [시나리오 10-1] 무한 스크롤 중 API 실패 후 재시도 성공 - 인기영화

User -> Main Page -> 스크롤하여 페이지 끝에 도달 -> 다음 페이지 API 실패 -> 토스트 에러 표시 -> 다시 스크롤하여 페이지 끝에 도달 -> 동일 페이지 재요청 성공 -> 영화 목록이 추가 표시된다

## [시나리오 11] 무한 스크롤 중 API 실패 시 스크롤 트리거 유지 - 검색

User -> Main Page -> 검색어 입력 -> 검색 결과 표시 -> 스크롤하여 페이지 끝에 도달 -> 다음 페이지 API 실패 -> 토스트 에러 표시 -> 스크롤 트리거가 사라지지 않고 유지된다

## [시나리오 11-1] 무한 스크롤 중 API 실패 후 재시도 성공 - 검색

User -> Main Page -> 검색어 입력 -> 검색 결과 표시 -> 스크롤하여 페이지 끝에 도달 -> 다음 페이지 API 실패 -> 토스트 에러 표시 -> 다시 스크롤하여 페이지 끝에 도달 -> 동일 페이지 재요청 성공 -> 검색 결과가 추가 표시된다
...
- [x] 무한 스크롤 중 API 실패 시 해당 페이지를 건너뛰지 않고 재시도할 수 있다
