import { ErrorCode, ERROR_CODE, ERROR_MESSAGE, ERROR_MESSAGE_FORMAT } from '../constants/errors';
...
export const isValidErrorCode = (code: unknown): code is keyof ErrorCode =>
  typeof code === 'string' && ERROR_CODE.hasOwnProperty(code);
...
const createErrorParams = ({ code, payload }: ErrorInfo, value: unknown): [string, ErrorOptions] => {
...
export class CustomError extends Error {
  constructor(info: ErrorInfo, value?: unknown) {
    ...
    this.name = isValidErrorCode(info.code) ? info.code : ERROR_CODE.UNEXPECTED_ERROR;
  }
}
