...
describe("인기 있는 영화 페이지 E2E 테스트", () => {
  ...
  it("마지막 페이지에서는 추가 데이터 요청이 발생하지 않아야 한다.", () => {
    ...
    cy.scrollTo("bottom");
    cy.wait("@getPopularMovies");
    cy.get(".thumbnail-list > li.item").should("have.length", 60);
  ...
  context("데이터 준비 및 상태 표시 테스트", () => {
    it("로딩 중일 때 스켈레톤 UI가 표시되어야 한다.", () => {
      ...
      // 스켈레톤 UI가 제거된 후 영화 목록이 렌더링되어야 함
      cy.get(".skeleton-item").should("not.exist");
    });

    it("통신 중 에러(500) 발생 시 에러 처리가 되어야 한다", () => {
      cy.intercept(
        {
          method: "GET",
          url: /^https:\/\/api\.themoviedb\.org\/3\/movie\/popular*/,
    ...
    it("로딩 시간이 10초가 넘어가면 에러 처리가 되어야 한다", () => {
      cy.intercept(
        {
          method: "GET",
          url: /^https:\/\/api\.themoviedb\.org\/3\/movie\/popular*/,
