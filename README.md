...
## 영화 상세 정보 조회
- [] 영화를 누르면 해당 영화에 대한 상세 정보가 모달로 나타난다.
- [] 모달에 있는 내용은 영화 썸네일, 제목, 개봉일, 장르, 평균 평점, 별점 매기기, 줄거리이다.
- [] 닫기 버튼을 누르거나 ESC를 누르면 모달 창을 닫을 수 있다.

## 별점 매기기
- [] 영화에 대한 별점을 줄 수 있다.
- [] localstorage에 내가 매긴 별점이 저장된다.
- [] 새로고침이나 모달 창을 닫아도 내가 매긴 별점이 사라지지 않는다.
- [] 별점은 총 5개로 구성된다.
- [] 한 개당 2점이며 1점 단위는 고려하지 않는다.
- [] 내가 선택한 별점에 따라 점수가 변한다.
- [] 점수에 따라 나타나는 문구가 달라진다.
  - [] 2점: 최악이예요
  - [] 4점: 별로예요
  - [] 6점: 보통이에요
  - [] 8점: 재미있어요
...
- [] 모달 창에 해당하는 영화 상세 정보를 불러오지 못했다면 모달 창 내부에 `영화 상세 정보를 불러오지 못했습니다. 다시 시도해주세요.` 라는 문구를 띄운다.
