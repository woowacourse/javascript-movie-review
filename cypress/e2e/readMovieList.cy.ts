...
describe('Fixture를 이용한 테스트', () => {
  ...
  it('영화의 제목, 평점, 포스터가 올바르게 화면에 보여야 한다', () => {
    cy.wait('@getPopularMovies').then((interception) => {
      cy.get('.thumbnail-list li').first().find('strong').should('have.text', '미키 17')

      cy.get('.thumbnail-list li .sm-star-box').first().find('p').should('have.text', ' 7')
      ...
      cy.get('.thumbnail-list li')
        ...
        .should(
          'have.attr',
          'src',
          'https://image.tmdb.org/t/p/w500/7KghOYtsxFquUuw4THbARsSEo6g.jpg'
        )
    })
  })

  it('스크롤 시 다음 영화 목록이 20개 더 출력된다.', () => {
    cy.wait('@getPopularMovies').then((interception) => {
      cy.intercept(
        {
          method: 'GET',
      ...
      cy.wait('@getMoreMovieList').then((interception) => {
        ...
        cy.get('.thumbnail-list li').eq(20).find('strong').should('have.text', '짱구')
      })
    })
  })
})
